...
\mnote{It is the assumption that message sending is costless that differentiates this model from signaling models, and that makes it \emph{cheap} talk.}
To what extent would the sender reveal her information?
When sender and receiver has identical preferences, the sender would of course reveal all the information she has, so that receiver can make an informed decision.
When their preferences are completely opposed, the sender would not send any message (or she babbles), since any information will be used for the receiver, against the sender.
...
\begin{itemize}
  ...
  \item The sender and the receiver has utility functions \(u(a, \theta) = -(a - \theta - b)^2\) and \(u(a, \theta) = -(a - \theta)^2\).
    The receiver tries to match the state, while sender always prefer the action higher then the state by \(b > 0\) (\underline{b}ias).
  \item The sender observes the state \(\theta\) and chooses a communication strategy \(\sigma: \Theta \to M\), where \(M\) is the set of possible messages.
  \item Receiver observes the communication strategy \(\sigma\) and a resulting message \(m\), and then chooses an action \(\hat{a}(m)\).
\end{itemize}

Let us first consider the receiver's strategy for a fixed communication strategy \(\sigma\).
Upon observing a message from sender, the receiver forms a posterior belief about the state and chooses \(a\) to maximize her expected payoff.
Because of the quadratic utility specification, her best action would simply be the posterior mean.
However, to later check whether the sender has a profitable deviation, we also need to specify what the receiver would do upon observing a message that is never sent under \(\sigma\), an \vocab{off-path} message.
In these cases, we simply assume the receiver chooses one of the action that she would also choose for an on-path message.
...
Observe first that a perfectly revealing equilibrium does not exist, for at each state \(\theta\) sender has a profitable unilateral deviation from reporting the truth \(\theta\) to reporting \(\theta + b\), knowing sender would then choose action \(a = \theta + b\).

Thus consider the other extreme, a perfectly uninformative equilibrium, where the sender sends the same message independent of the state.
Given sender's strategy, the receiver would gain no information about the state from the message and thus choose action \(a = 1 / 2\), which is the prior mean of \(\theta\).
Given our assumption that receiver would still choose \(a = 1 / 2\) upon receiving any other message from the sender, the sender would trivially have no incentive to deviate.
Thus the perfectly uninformative equilibrium --- called a \vocab{babbling equilibrium} --- always exists.
In this equilibrium, the sender babbles; no information is conveyed.
...
Perhaps the most natural intermediate case where only partial information is communicated, is the sender sending a message telling the receiver whether \(\theta\) exceeds some thresholds, say \(\theta_1\).
With this communication strategy, the receiver would then choose \(a = (\theta_1 + 1) / 2\) if \(\theta\) exceeds the threshold and \(a = \theta_1 / 2\) otherwise.
To ensure that the sender has no incentives to deviate, it is by continuity necessary for the sender to be indifferent between sending either message when she sees \(\theta_1\):
\[
...
Thus, if \(b \leq 1 / 2\), we have shown that there exists an equilibrium where sender reports whether \(\theta\) exceeds \(\theta_1 \coloneq 1 / 2 - 2b\).
...
\]
consider the sender's strategy of reporting \(m_n\) whenever \(\theta \in [\theta_{n - 1}, \theta_n]\).
...
\]
From the last expression we see each interval is longer than the one before it by \(2b\), so for fixed \(b\), the number of intervals \(N\) is bounded, with the largest possible \(N\) decreasing in \(b\).
...
What happens if we give commitment power to the sender?
...
This is in contrast to the cheap talk model, where we require the sender's disclosure policy to be optimal conditional on her information, no matter what information she might receive, i.e., the policy must be \emph{interim} incentive compatible.
Now, the agent optimizes purely from an ex ante standpoint, knowing that she will follow through.

\begin{itodo}
compare with stackelberg competition.
\end{itodo}

When is such an assumption realistic?
An example is a grading or ranking system: an institution picks and makes public an evaluation procedure before collecting information.
...
But the sender can also be interpreted metaphorically, as Nature choosing what information about the state is available to the receiver.
...
The last representation is perhaps most revealing of the fact that the receiver gains information by observing a signal that correlates with the state.

Our new game can be summarized as follows:
\begin{itemize}
  \item There is a finite set of payoff-relevant states.
  \item The sender and receiver respectively have continuous utility functions \(v(a, \theta)\) and \(u(a, \theta)\) and a common prior \(\mu_0 \in \Delta(\Theta)\) that puts strictly positive probability on each state.
    \mnote{
...
Let us first fix an information structure \(\pi\) and a signal realization \(s\), and ask what the receiver would do.
...
These beliefs are stochastic since with the same \(\pi\), the receiver can observe potentially different random signals, but the distribution of the beliefs are completely determined by the information structure \(\pi\).
Instead of thinking about choosing \(\pi\), let us thus think how the sender would choose a distribution over posteriors directly.
...
For instance, it is not possible for the receiver to have a posterior mean higher than that of the prior after seeing every signal realization --- for if she knew she would have beliefs with higher means after getting \emph{any} information, she would have revised her beliefs upwards before getting the information.
...
We say \(\tau\) is \vocab{Bayes plausible} if it satisfies the condition above.
...
We may now rephrase the Sender's problem as
\[
...
\begin{proposition}
  Write \(\conv \hat{v}\) doe the convex hull of the graph of \(\hat{v}\), and let \(V(\mu) \coloneq \sup \{z: (\mu, z) \in \conv \hat{v}\) denote the concave closure of \(\hat{v}\).
...
\begin{proof}
  ...
  Interpreting the weights as probabilities and we see that there is a distribution over these posteriors \(\tau\) that averages to \(\mu\).
