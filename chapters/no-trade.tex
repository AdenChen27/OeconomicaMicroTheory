...
\input{chapters/farkas}


\section{The No-Trade Theorem}
Voluntary trading between individuals is possible only when the agents are different in some way.
...
Say a trade \(\gamma\) is \vocab{feasible} if no outside resource is needed to facilitate it:
\[
...
\begin{remark}
  ...
  To see this, suppose there are two types of player \(A\), \(t_0^A\) and \(t_1^A\), and both \(A\) and \(B\) know that \(A\) finds \(\gamma\) acceptable iff \(A\) she is of type \(t_1^A\).
  ...
  In doing so, he analyses whether a trade will be accepted at each possible type profile \(t\) and weighs the corresponding payoffs using the probabilities of the types.
  Instead of going through such trouble, we can simply impose that a trade must be acceptable at all \(t\)'s --- and doing so is without loss, since we can also set the payoffs to be \(0\) when a trade is not accepted at some profile --- so that interim payoffs have the simple form above.
\end{remark}
...
\begin{lemma}\label{lem:farkas-variant-2}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item There exists \(j \in J\) and \(y \in \bR^m\) such that \(yA \geq 0\), for all \(j\), with strict inequality for column \(j\).
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  Then \(x \coloneq \sum_j x^j\) satisfies (i).
\end{proof}
